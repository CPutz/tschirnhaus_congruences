...
class PolGenerator
{
    long *divisors; // divisor for each a_k (A_k)
...
public:
    PolGenerator(int const n, int const p, std::vector<long> const &fixedcoefficients,
        long *divisors, long const discriminant, std::fstream &treestream);
    ~PolGenerator();
